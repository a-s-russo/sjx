...
create_ts_comp <- function(series, dataItem) {
  ...
  if (!is.null(ABS_seas_ts))  # Series might not be published in ABS data
    results <-
    append(results, list("ABS_seas" = ABS_seas_ts))
  if (!is.null(ABS_tren_ts))  # Series might not be published in ABS data
    results <-
...
#' create_tsdf_comp Create a long data frame for one group of original, seasonally adjusted and trend series corresponding to a single data item description in a downloaded and cleaned table from an ABS publication in addition to seasonally adjusted and trend series from X-13ARIMA-SEATS and JDemetra+
#'
#' @param series A list returned from download_ts containing series and their metadata
#' @param dataItem The data item description for which results are to be returned
...
create_tsdf_comp <- function(series, dataItem) {
  ...
  results <- data.frame("period" = time(dataItem_ts$ABS_orig))
  if (!is.null(dataItem_ts$ABS_seas)) # Series might not be published in ABS data
    results <-
  ...
  if (!is.null(dataItem_ts$ABS_tren)) # Series might not be published in ABS data
    results <-
...
#' create_tsdf_table Create a list of long data frames for all original, seasonally adjusted and trend series in a downloaded and cleaned table from an ABS publication in addition to seasonally adjusted and trend series from X-13ARIMA-SEATS and JDemetra+
#'
#' @param series A list returned from download_ts containing series and their metadata
#' @details Requires the data output from download_ts. Null values appear in place of non-existent series in the list returned.
...
#' run_RJD Run JDemetra+ to generate seasonally adjusted and trend estimates for a time series object
#' 
...
#' run_X13 Run X-13ARIMA-SEATS to generate seasonally adjusted and trend estimates for a time series object
#' 
#' @param ts A time series object containing original estimates
#' @details Seasonal adjustment and trending are performed using X-13 rather than SEATS
