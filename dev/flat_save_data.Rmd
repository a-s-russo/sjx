...
download_ts <- function(url) {
  ...
  partial_series <-
    NULL # Remove these series (X-13 for both RJDemetra+ and 'seasonal' cannot handle missing observations, so remove)
  long_series <-
...
#' @param series The list returned from `download_ts` containing series data and
#' metadata
#' @param dataItem A data item description for which results are to be returned
#' 
...
create_tsdf_comp <- function(series, dataItem) {
  ...
  # Merge on seasonally adjusted series generated by X-13ARIMA-SEATS and JDemetra+
  X13_seas_series <-
  ...
  # Merge on trend series generated by X-13ARIMA-SEATS and JDemetra+
  X13_tren_series <-
    data.frame("period" = ABS_orig_series$period,
...
#' will not contain them either. (A NULL object will be returned instead if the
#' plot is empty; that is, if none of these series are published or couldn't be
#' generated.)
#' 
...
#' returned instead if the plot is empty; that is, if none of these series are
#' published or couldn't be generated.)
#' 
