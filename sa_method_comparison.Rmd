...
spreadsheets under the 'Data downloads' tab of the desired publication.

An example file is Table 1 (Inventories, chain volume measures) in the March
...
* The first tab of the spreadsheet file ('Index') contains metadata about the time
series, such as the series type, units and frequency.

* The second tab ('Data1') contains the original, seasonally adjusted and trend
...
The following function plots the seasonally adjusted or trend series produced
by X-13ARIMA-SEATS or JDemetra+ against the ABS versions and outputs the graphs
to a PDF file. It requires the data output from the download function above,
and calls the time series generation functions above during execution.
